            ...
            if (user.last_message == 'prompt') {
                ...
                if (identifyPositive(text)) {
                    User.findOneAndUpdate({
                        ...
                        'last_message': 'suscription_confirmation',
                        'status': 'subscribed'
                    }, function(err, doc) {
                        if (err) {
                            reply_text = {
                            ...
                            reply_text = {
                                text: '¡Perfecto! Ya quedaste suscrito a todas las notificaciones de goles y resultados finales de partidos.'
                            }
                        }
                    ...
                    User.findOneAndUpdate({
                        ...
                        'last_message': 'no_suscription_confirmation',
                        'status': 'undefined'
                    }, function(err, doc) {
                        if (err) {
                            reply_text = {
                ...
                User.findOneAndUpdate({
                    ...
                    if (identifyGratefulness(text)) {
                        reply_text = {
                            text: ':D Ahora sí, a disfrutar esta Copa América ' + user.name + "!"
                        }
                    } else {
                        reply_text = {
                            text: user.name + 'recuerda que puedes conocer más sobre esta copa américa en http://cappcentenario.com/informacion/'
                        }
                    }
                    replyMessage(reply, reply_text);
                });
                ...
                User.findOneAndUpdate({
                    ...
                    if (identifyGratefulness(text)) {
                        reply_text = {
                            text: 'Vale :(. Igual te voy a extrañar ' + user.name + '.'
                        }
                    } else {
                        reply_text = {
                            text: 'No entiendo lo que me dices ' + user.name + ', pero espero me disculpes si fueron molestas las notificaciones.'
                        }
                    }
                    replyMessage(reply, reply_text);
                });
                ...
                if (true) {
                    ...
                    user.save(function(err, obj) {
                        if (err) {
                            reply_text = {
                                text: 'Ups me chispotié. Algo salió mal.'
                            };
                            replyMessage(reply, reply_text);
                        }
                        var reply_payload = {
            ...
            if (user.status != 'subscribed') { //Only subscribe unsubscribed users
                ...
                user.save(function(err, obj) {
                    if (err) {
                        ...
                        reply_text = {
                            text: 'Parece que ya estabas registrado para recibir las notificaciones. ¡Sigue disfrutando de lo mejor de esta Copa América!'
                        }
                    }
                    replyMessage(reply, reply_text);
                });
            ...
            if (user.status == 'subscribed') {
                ...
                user.save(function(err, obj) {
                    if (err) {
                        ...
                        reply_text = {
                            text: 'He tachado tu nombre de la lista de suscritos. No te molestaré con más notificaciones.'
                        }
                    }
                    replyMessage(reply, reply_text);
                });
                ...
                user.save(function(err, obj) {
                    if (err) {
                        ...
                        reply_text = {
                            text: 'Entendido. No te enviaré notificaciones en esta Copa América.'
                        }
                    }
                    replyMessage(reply, reply_text);
                });
                ...
                user.save(function(err, obj) {
                    if (err) {
                        ...
                        reply_text = {
                            text: 'Ya habías eliminado tu suscripción a nuestras notificaciones.'
                        }
                    }
                    replyMessage(reply, reply_text);
                });
...
function addUser2(payload, reply) {
    bot.getProfile(payload.sender.id, function(err, profile) {
        ...
        var text = {
            text: '¡Perfecto! Te enviaré notificaciones de goles y resultados finales de partidos.'
        }
